...
def recommend_food(input_value, top_n=10, by='name'):
    ...
    The fuzzy match recipe is prioritized to ensure varieties of the searched item are included.
    """
    if by == 'name':
    ...
    # Sort indices by cosine similarity and take the top N (guarantees the exact match is first)
...
def recommend_by_nutrition(calories=None, protein=None, fat=None, carbs=None, top_n=10):
    ...
    Accuracy Enhanced: Only calculates difference for metrics provided to ensure closest possible match.
    """
    df_copy = df.copy()
...
def recommend_pair(food_name, top_n=10): 
    ...
    Enhanced: Ensures the paired recipes are distinct from the source recipe.
    """
    ...
    if not possible_pairs:
        ...
        # Ensure we only sample recipes that are NOT the base recipe
        ...
        if not same_cuisine_recipes.empty:
            # Use replace=False if possible, but use True if sample size is large
            sampled_recipes = same_cuisine_recipes['recipe_name'].sample(min(top_n, len(same_cuisine_recipes)), replace=False).tolist()
