...
app = FastAPI(title="Food Recommendation System (Enhanced)")
...
# 4. Recipe Type Classification (New Feature)
recipe_type_keywords = {
    'Dessert': ['cake', 'cookie', 'brownie', 'ice cream', 'pudding', 'pie', 'tart'],
    'Breakfast': ['pancake', 'waffle', 'omelette', 'scrambled egg', 'smoothie', 'oats'],
    'Soup': ['soup', 'broth', 'stew', 'chowder'],
    'Main Course': ['curry', 'steak', 'roast', 'casserole', 'biryani', 'masala', 'pasta'],
    'Snack': ['sandwich', 'fries', 'taco', 'momo', 'roll', 'burger']
}
...
def recommend_food(input_value, top_n=10, by='name'):
    ...
    if by == 'name':
        # Use lowercased list for process.extractOne
    ...
    # MODIFICATION: Include the input recipe itself, which has the highest similarity (1.0), 
    # and take the top N results.
    similar_indices = cosine_sim.argsort()[-top_n:][::-1] 
    
    return df.iloc[similar_indices][['recipe_id','recipe_name','image_url','cuisine_type','region_type','veg_nonveg','aver_rate', 'recipe_type']]
...
def recommend_by_nutrition(calories=None, protein=None, fat=None, carbs=None, top_n=10):
    ...
    # Use mean if target is None to prevent large arbitrary differences
    cal_target = calories if calories is not None else df_copy['calories'].mean()
    prot_target = protein if protein is not None else df_copy['protein'].mean()
    fat_target = fat if fat is not None else df_copy['fat'].mean()
    carbs_target = carbs if carbs is not None else df_copy['carbohydrates'].mean()

    df_copy['nutrition_score'] = (
        abs(df_copy['calories'] - cal_target) +
        abs(df_copy['protein'] - prot_target) +
        abs(df_copy['fat'] - fat_target) +
        abs(df_copy['carbohydrates'] - carbs_target)
    )
...
def recommend_pair(food_name, top_n=10): # Increased top_n here
    ...
    if not possible_pairs:
        ...
        # Sample other recipes from the same cuisine
        same_cuisine_recipes = df[df['cuisine_type'] == cuisine]['recipe_name'].sample(min(top_n, 10)).tolist()
        possible_pairs.extend(same_cuisine_recipes)
        
    
    # 3. Filter the main DataFrame to get the details of the paired recipes
    # Use fuzzy matching for pairs in case the pair list contains exact recipe names
    pair_df_list = []
